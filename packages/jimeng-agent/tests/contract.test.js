...
describe('jimeng-agent/contract — agentPrompt assembly', () => {
  it('assembles seedance2.0 header with adjacent asset-id line and no user prompt', () => {
    const out = normalizeAskArgs(baseArgs({
      model_version: 'seedance2.0',
      prompt: '',
    }));
  ...
  it('inserts tool-generated asset-id then --- separator before the user prompt', () => {
    const out = normalizeAskArgs(baseArgs({
  ...
  // assetId is always generated by the CLI; never a user input flag.
  it('always auto-generates a 16-char hex asset id', () => {
    const a = normalizeAskArgs(baseArgs({ prompt: '' }));
    const b = normalizeAskArgs(baseArgs({ prompt: '' }));
    expect(a.assetId).toMatch(/^[0-9a-f]{16}$/);
    expect(b.assetId).toMatch(/^[0-9a-f]{16}$/);
    expect(a.assetId).not.toBe(b.assetId);
    expect(a.agentPrompt).toContain(`资产编号：${a.assetId}`);
  });

  it('still inserts the --- separator when the prompt is whitespace-padded', () => {
    const out = normalizeAskArgs(baseArgs({ prompt: '   ' }));
    expect(out.agentPrompt.endsWith('\n\n---\n\n   ')).toBe(true);
    expect(out.agentPrompt).toContain(`资产编号：${out.assetId}`);
  });
});
...
describe('jimeng-agent/contract — canonical payload shape', () => {
  ...
  it('normalizes --submit 0/1 into a boolean gate for formal generation', () => {
    expect(normalizeAskArgs(baseArgs({ submit: 0 })).submit).toBe(false);
    expect(normalizeAskArgs(baseArgs({ submit: 1 })).submit).toBe(true);
    expect(normalizeAskArgs(baseArgs({ submit: 'true' })).submit).toBe(true);
    expect(() => normalizeAskArgs(baseArgs({ submit: 2 }))).toThrow(ArgumentError);
  });

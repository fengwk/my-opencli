...
/**
 * Assemble the exact `agentPrompt` string the future LLM-facing submit step
 * will hand to jimeng. The format is:
 *
 *   <modelPrefix>，你必须严格按照下面的提示词内容生成1个<ratio>的<duration>s视频
 *   <BLANK LINE>
 *   <original prompt if non-empty>
 *
 * The blank line is exactly two LF characters when the prompt is non-empty;
 * absent when it is empty. No trailing newline.
 */
function assembleAgentPrompt({ modelVersion, ratio, duration, prompt }) {
  ...
  if (prompt.length > 0) {
    // Two LF characters — explicitly not a platform-specific newline.
    text += '\n\n' + prompt;
  }
